...
The `input` directory has the LAMMPS inputs. Sample Slurm scripts have also been provided for reference. 

We supply a LAMMPS parameter file `data.begin.bz2`. This compressed file must be uncompressed before any test runs. It contains coordinates and velocities of ~0.75 million atoms in a cubic unit cell, ~20 nm on a side, representing a 35% LiCl solution pre-equlibrated to 300 K and 1 atm. Larger unit cells for the "medium" and "large" benchmarks are generated programmatically from this file.

Only the medium and large LAMMPS benchmarks are required, however we provide input files for the small, medium, large, and xlarge benchmarks. They are named `small.in`, `medium.in`, `large.in`, and `xlarge.in`. For `medium.in`, `large.in`, and `xlarge.in`, the unit cell decribed above is replicated to create larger cubic cells (via the LAMMPS `replicate` commands).  

The `NLR_results` directory has reference results for validation. 
...
LAMMPS can be built by following the instructions at [https://lammps.sandia.gov/doc/Install.html](https://docs.lammps.org/Build.html). For systems with GPUs, a version should be built with the LAMMPS GPU or KOKKOS package with the default precision. If the Offeror's chip doesn't support these packages, please contact NLR as soon as possible to discuss possiblilties for using other package.  
...
Each benchmark case should be run on 1, 2, 4, 8... nodes, up to the maximum number of nodes available for testing. It is possible that some benchmarks may not run on small node counts due to memory constraints, such as for the `large` and `xlarge` benchmarks on a small amount of Accelerated nodes. `Out of Memory` should be reported for any of these cases in the Spreadsheet response. It is also possible that the optimal speed is achieved when the number of MPI ranks per node is smaller than the number of CPU cores per node and in this case, the Offeror may be able improve performance by varying the number of MPI ranks per node to find a more optimal value. The "# cores" reported should reflect the number of `physical` CPU cores hosting independent threads of execution.
...
A script named `validate.sh` is provided to validate system temperature and total energy data from the output of each of these runs against those obtained by NLR. The Offeror should run this script with the first argument as the path to the output file of a validation run, and the second argument the path to the provided directory `NLR_results` containing reference data. The usage is:

`./validate.sh PATH_TO_OUTPUT/lammps_output PATH_TO_NLR_RESULTS/`

(Example: `./validate.sh numerical_std/medium_numerical_test.log NLR_results/` ) 

This will produce a file called `thermo.dat` that contains time, temperature, and energy output for the run; and, a file `rms_errors.dat` that contains validation information. The first line in `rms_errors.dat` gives the average temperature and energy for the run. These should have values of approximately 300 and -8.9e+07, respectively. The second line gives the relative root-mean-squared deviation between the output provided by NLR (in `NLR_thermo.dat`) and the output from the test run. Reasonable relative deviations are less than 10<sup>-3</sup> for temperature and 10<sup>-5</sup> for energy. The validation script returns either "run validated" or "run not validated" on the third line of `rms_errors.dat`, depending on whether deviations are less than or greater than these thresholds, respectively.
