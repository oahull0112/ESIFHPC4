...
BENCHMARK_DIR=/scratch/${USER}/amr-wind-benchmark/amr-wind-benchmark-gpu-aware
...
cat >build-amr-wind-benchmark-gpu-aware.sh <<'EOL'
#!/bin/bash -l
set -ex
module load cuda
...
# Build AMR-Wind
bash build-amr-wind-benchmark-gpu-aware.sh

# Submit run script
sbatch -J amr-wind-benchmark-gpu-1 -N 1 run-amr-wind-benchmark-gpu-aware.sh; for i in {1..5}; do sbatch -J amr-wind-benchmark-gpu-$((2**i)) -N $((2**i)) run-amr-wind-benchmark-gpu-aware.sh; done
