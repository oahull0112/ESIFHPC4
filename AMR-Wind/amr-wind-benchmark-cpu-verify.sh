...
BENCHMARK_DIR=/scratch/${USER}/amr-wind-benchmark/amr-wind-benchmark-cpu-verify
...
cat >build-amr-wind-benchmark-cpu-verify.sh <<'EOL'
#!/bin/bash -l
set -ex
...
# Generate run script
cat >run-amr-wind-benchmark-cpu-verify.sh <<'EOL'
#!/bin/bash -l
...
# Build AMR-Wind
bash build-amr-wind-benchmark-cpu-verify.sh

# Submit run script
sbatch -J amr-wind-benchmark-cpu-verify-4 -N 4 -p hbw run-amr-wind-benchmark-cpu-verify.sh
