...
int main(int argc, char** argv)
{
    ...
    int ierr;
    ierr=cudaMalloc((void**)&d_a, sizeof(real)*N);if (ierr > 0) printf("d_a ierr %d\n",ierr);
    ierr=cudaMalloc((void**)&d_b, sizeof(real)*N);if (ierr > 0) printf("d_b ierr %d\n",ierr);
    ierr=cudaMalloc((void**)&d_c, sizeof(real)*N);if (ierr > 0) printf("d_c ierr %d\n",ierr);
    ...
    printf(" Total time %10.4g seconds\n",mysecond()-st);
