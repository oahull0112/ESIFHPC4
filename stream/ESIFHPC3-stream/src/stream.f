...
C     .. External Functions ..
      ...
      INTEGER(int8) checktick,realsize
      EXTERNAL mysecond,checktick,realsize
!$    INTEGER(int8) omp_get_num_threads
...
C     ..
C     .. Arrays in Common ..
C     DOUBLE PRECISION a(ndim),b(ndim),c(ndim)
...
C     ..
      ...
      if (command_argument_count() >= 1)then
              call get_command_argument(1,sizestr)
              read(sizestr,*)n
              ndim=n
      endif
      if (command_argument_count() > 1)then
...
*-------------------------------------
* INTEGER(int8) FUNCTION dblesize()
*
...
* number occupies.
*
      INTEGER(int8) FUNCTION realsize()
      use numz
*     IMPLICIT NONE

C     .. Local Scalars ..
      DOUBLE PRECISION result,test
      INTEGER(int8) j,ndigits
C     ..
C     .. Local Arrays ..
...
*
      INTEGER(int8) FUNCTION checktick()
              use numz
*     IMPLICIT NONE
